...
const (
	contextKeyCertificate contextKey = iota
	contextKeyHost
	contextKeyRemoteAddr
)

// CertificateFromContext returns the client X.509 certificate from the context, or nil if absent.
func CertificateFromContext(ctx context.Context) *x509.Certificate {
	v, _ := ctx.Value(contextKeyCertificate).(*x509.Certificate)
	return v
}

// HostFromContext returns the HTTP Host header value from the context.
func HostFromContext(ctx context.Context) string {
	v, _ := ctx.Value(contextKeyHost).(string)
	return v
}

// RemoteAddrFromContext returns the remote address of the client from the context.
func RemoteAddrFromContext(ctx context.Context) string {
	v, _ := ctx.Value(contextKeyRemoteAddr).(string)
	return v
}
