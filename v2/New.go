            ...
            for _, tk = range strings.Split(strings.Trim(path,"/"),"/") {
               if tk!="" {
                  if (tk[0]=='{') && (tk[len(tk)-1]=='}') {
                     re += "([^/]+)/"
                     parmcount++
                     SwaggerParameter, err = GetSwaggerType(method.Type.In(parmcount+1))
                     if err != nil {
                        return nil, err
                     }
...
//                     if (SwaggerParameter.Items != nil) || (SwaggerParameter.CollectionFormat!="") || (SwaggerParameter.Schema.Required != nil) {
            ...
            if postdata != "" {
               ...
               for k, postField = range postFields {
                  parmcount++
                  pt[k] = method.Type.In(parmcount+1)
...
*/
            ...
            // In v1, every service method must have context.Context as first parameter (after receiver).
            // So: num = 1 (receiver) + 1 (context.Context) + parmcount (service params)
            if num < 2 || method.Type.In(1) != reflect.TypeOf((*context.Context)(nil)).Elem() {
               return nil, errors.New(fmt.Sprintf("Method %s must have context.Context as first parameter", methodName))
            }
            if (parmcount + 2) != num {
               return nil, errors.New(fmt.Sprintf("Wrong parameter count at method %s: expected %d parameters (context.Context + %d service params), got %d total inputs",methodName,parmcount+1,parmcount,num))
            }
